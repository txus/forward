...
template <typename T, typename D> Tensor<T, D> arange(T start, T end, T step) {
  ...
  uint32_t n = n_elements;
  metal::launch(fn_name, n_elements,
                {to_buffer(out), to_bytes(start), to_bytes(step), to_bytes(n)});

  return out;
}

template Tensor<bfloat16, METAL> arange<bfloat16, METAL>(bfloat16 start, bfloat16 end, bfloat16 step);
